...
import { Clock, Users, CalendarDays, ArrowLeftRight, AlertCircle, User } from 'lucide-react';
...
export function ShiftOverview() {
  ...
  // For admins, show all schedules. For regular users, show only their own shifts
  const { schedules, isLoading: schedulesLoading } = useShiftSchedules(undefined, isAdmin ? undefined : profile?.id);
  ...
  // For non-admins, filter swaps to only show their own
  const userSwaps = isAdmin ? pendingSwaps : pendingSwaps.filter(s => 
    s.requester_id === profile?.id || s.target_employee_id === profile?.id
  );
  ...
  return (
    <div className="space-y-6">
      {/* User's shift info banner for non-admins */}
      {!isAdmin && (
        <Card className="border-primary/30 bg-primary/5">
          ...
          <CardContent>
            {schedules.length === 0 ? (
              <p className="text-muted-foreground text-sm">No shifts scheduled for you this week.</p>
            ) : (
              <div className="space-y-2">
                {schedules.slice(0, 5).map((schedule) => (
                  <div key={schedule.id} className="flex items-center justify-between p-3 rounded-lg border bg-card">
                    <div>
                      <p className="font-medium">{format(new Date(schedule.schedule_date), 'EEEE, MMM d')}</p>
                      <p className="text-sm text-muted-foreground">
                        {schedule.shift_type?.start_time} - {schedule.shift_type?.end_time}
                      </p>
                    </div>
                    <Badge style={{ backgroundColor: schedule.shift_type?.color }} className="text-white">
      ...
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium">{isAdmin ? "Today's Shifts" : "Your Today"}</CardTitle>
            <CalendarDays className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">{todaySchedules.length}</div>
            <p className="text-xs text-muted-foreground">{isAdmin ? 'employees scheduled' : 'shift(s) today'}</p>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium">{isAdmin ? 'Pending Swaps' : 'Your Swaps'}</CardTitle>
      ...
      {/* Today's Schedule - Admin only for full view */}
      {isAdmin && (
        <Card>
          <CardHeader>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{isAdmin ? "This Week's Coverage" : "Your Week"}</CardTitle>
            ...
            {[...Array(7)].map((_, i) => {
              ...
              return (
                <div
                  ...
                  } ${daySchedules.length > 0 && !isAdmin ? 'ring-2 ring-primary/30' : ''}`}
                >
                  ...
                  <p className="text-xs text-muted-foreground">
                    {daySchedules.length} {isAdmin ? 'shifts' : daySchedules.length === 1 ? 'shift' : 'shifts'}
                  </p>
                </div>
              );
            })}
        ...
        <Card className="border-warning">
          <CardHeader>
            <CardTitle className="flex items-center gap-2 text-warning">
              ...
              {isAdmin ? 'Pending Swap Requests' : 'Your Swap Requests'}
            </CardTitle>
          </CardHeader>
          <CardContent>

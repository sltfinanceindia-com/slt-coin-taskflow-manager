    ...
    <div className="space-y-6">
      ...
      <Card className="bg-gradient-to-br from-emerald-50 to-blue-50 border-2 border-emerald-200">
        <CardHeader>
          ...
          <CardDescription>Real-time coin rate and market statistics</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>Add a new rate entry to the system</CardDescription>
            </div>
            <Button
              variant={isAddingRate ? 'outline' : 'default'}
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="volume_24h">24h Volume (USD)</Label>
                  <Input
                    id="volume_24h"
                    type="number"
                    step="0.01"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Complete history of all rate updates</CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <div className="space-y-2">
                    ...
                    <TableRow key={rate.id}>
                      ...
                      <TableCell>
                        {rate.change_percentage ? (
                          <Badge 
                            variant={Number(rate.change_percentage) >= 0 ? 'success' : 'destructive'}
                          >
                      ...
                      <TableCell>
                        {rate.volume_24h ? `$${Number(rate.volume_24h).toLocaleString()}` : '-'}
                      </TableCell>
                      <TableCell>
                        {rate.market_cap ? `$${Number(rate.market_cap).toLocaleString()}` : '-'}
                      </TableCell>
                      <TableCell>
                        {(rate.created_by as any)?.full_name || 'System'}
                      </TableCell>

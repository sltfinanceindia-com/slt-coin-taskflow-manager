    ...
    <section className="py-24 lg:py-32 overflow-hidden">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div className="text-center mb-16" initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
          <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-foreground mb-4">Integrates with Tools You Already Use</h2>
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">Connect TeneXA with your existing tech stack for a seamless workflow</p>
        </motion.div>
        ...
        <motion.div className="relative max-w-4xl mx-auto" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
          <div className="flex items-center justify-center mb-12">
            <motion.div
              className="relative w-24 h-24 rounded-2xl bg-gradient-to-br from-primary to-[#2E5F99] flex items-center justify-center shadow-xl shadow-primary/30"
              animate={{ boxShadow: ['0 0 30px hsla(213, 63%, 58%, 0.3)', '0 0 50px hsla(213, 63%, 58%, 0.5)', '0 0 30px hsla(213, 63%, 58%, 0.3)'] }}
          ...
          <motion.div className="grid grid-cols-2 sm:grid-cols-4 gap-4 lg:gap-6" variants={staggerContainer} initial="initial" whileInView="animate" viewport={{ once: true }}>
            {integrations.map((integration, index) => (
              <motion.div key={integration.name} variants={fadeInUp} whileHover={{ y: -5, scale: 1.02 }} className="group">
                <div className="p-6 rounded-2xl bg-background border border-border/50 hover:border-primary/50 transition-all duration-300 text-center hover:shadow-lg hover:shadow-primary/5">
                  <motion.div
          ...
          <div className="absolute inset-0 pointer-events-none overflow-hidden opacity-20">
            {[...Array(8)].map((_, i) => (
              <motion.div
                key={i}
                className="absolute w-px bg-gradient-to-b from-primary to-transparent"
                style={{ left: `${12.5 + i * 12.5}%`, top: '0', height: '40%' }}
                animate={{ opacity: [0.2, 0.5, 0.2], height: ['30%', '50%', '30%'] }}
